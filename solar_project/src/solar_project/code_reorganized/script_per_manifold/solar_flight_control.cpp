...
*/
 ...
 // #########################à INITIALIZATION OF KALMAN FILTER VARIABLES ###################
  ...
  float x_target_P1 = 0.35;  //Le coordinate sono del punto P1 = (0.3, 0.2) rispetto al world frame fittizio
  float y_target_P1 = 1.12; 
  float x_target_P2 = 2.10;
  float y_target_P2 = 1.12;
  float x_target_P1_rotated = 0.0; //Solamente in esperimenti in lab per far finat che i pannelli siano ruotati rispetto al frame
...
//Initialize drone position in world frame 
  float x_drone_I = -0.1;
  float y_drone_I = 1.56;
  
  //Initialize KF 
  KF.yaw = -0.0 * M_PI/180;
  KF.xh << 0.2, 0.3; //Inizializzare con valori retta a e c passante per i waypoint di inizio e fine pannello
  ...
  //If required rotate inertial frame--> Rotate target P1
   ...
   KF.translation_x = x_respect_frame; //Translation camera respect Inertial frame
   KF.translation_y = y_respect_frame;
 ...
 //Update KF with RGB observations 
  if  (control_P1_RGB_updated == true and control_P1_RGB_updated  == true or RGB_detection_count < 20)
  {
    //Update only if detected something
      KF.Kalman_Filter(x_target_P1_rotated, y_target_P1_rotated, x_target_P2_rotated, y_target_P2_rotated, target_point); //nel caso reale lo stato vero non è conosciuto quindi miodificare funzione 
...
//Update KF with Thermal  observations if available
  ...
  {
    //Update only if detected something
      KF.Kalman_Filter(x_target_P1_rotated, y_target_P1_rotated, x_target_P2_rotated, y_target_P2_rotated, target_point);
...
{
  double deltaLon = target.longitude - origin.longitude; //differenza sulla lon 
  double deltaLat = target.latitude - origin.latitude;  //differenza sulla lat

 //Salvo tutto nel vettore localOffse, qui richiamato NED, definito globalmente 
  deltaNed.y = deltaLat * deg2rad * C_EARTH; //Ottengo posizione y di offset --> cioè lo spostamento necessario su y 
  deltaNed.x = deltaLon * deg2rad * C_EARTH * cos(deg2rad*target.latitude); //Ottengo posizione x di offset --> cioè lo spostamento necessario su y 
