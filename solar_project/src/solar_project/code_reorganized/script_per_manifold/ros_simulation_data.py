...
def publish_img_hsv(img):
    camera_hsv_frame_pub = rospy.Publisher('OVTA/DEBUG_HSV', Image, queue_size=1)
   # msg = Image()
    msg_frame_HSV = CvBridge().cv2_to_imgmsg(img,"rgb8")
    #msg.data = msg_frame
    #Publish The image 
    camera_hsv_frame_pub.publish(msg_frame_HSV)

def publish_img_first_threshold(img):
    camera_first_frame_pub = rospy.Publisher('OVTA/DEBUG_FIRST_THRESHOLD', Image, queue_size=1)
   # msg = Image()
    msg_frame_first = CvBridge().cv2_to_imgmsg(img,"rgb8")
    #msg.data = msg_frame
    #Publish The image 
    camera_first_frame_pub.publish(msg_frame_first)

def publish_img_clustering_mask(img):
    camera_cluster_mask_pub = rospy.Publisher('OVTA/DEBUG_CLUSTERING', Image, queue_size=1)
   # msg = Image()
    msg_frame_mask = CvBridge().cv2_to_imgmsg(img,"rgb8")
    #msg.data = msg_frame
    #Publish The image 
