...
import numpy as np
from scipy.optimize import minimize, fmin_l_bfgs_b, basinhopping, brute
...
from ros_simulation_data import take_drone_camera_RGB_frame, take_drone_camera_frame, takeEnvObservations, take_drone_camera_RGB_frame_simulation, take_drone_camera_frame_simulation
...
def publish_thermal_default_param_while_waiting():
    ...
    #Upload also Thermal Values while waiting starting of the thermal optimization
...
def publish_default_HSV_param_while_waiting():
    H_MIN_default = rospy.get_param("/RGB_params/H_MIN")
    S_MIN_default = rospy.get_param("/RGB_params/S_MIN")
    V_MIN_default = rospy.get_param("/RGB_params/V_MIN")
    H_MAX_default = rospy.get_param("/RGB_params/H_MAX")
...
def image_pre_processing(Op, sd, RGB, image, drone_obj):
    ...
    #publish_thermal_default_param_while_waiting()
...
def  RGB_optimization(args):
    ...
    if (Op.test == True):
        ...
        #Load another image to make congruent the dimension 
        ...
        # plt.show()
        save_path ='/home/dji/DATA/dji_ws/user_image_selection_after_opt/Optimized_parameters_img/'
        filename = save_path + 'RGB_opt_img' + '.png'
        print(filename)
        cv2.imwrite(filename, clustered_image)

        #Save the final_mask used for correlation in next 
        save_path ='/home/dji/DATA/dji_ws/user_image_selection_after_opt/correlation_mask/'
        filename = save_path + 'RGB_correlation_mask' + '.png'
        print(filename)
        cv2.imwrite(filename, Op.corr_mask)
    ...
    clustered_image = image_pre_processing(Op, sd, RGB, image, drone_obj)
    return clustered_image, res, RGB_opt_flag, Op.corr_mask
...
###############################################
    ...
    #publish_default_HSV_param_while_waiting()
...
def Thermal_optimization(args):
    ...
    Op_thermo.Thermo_gaussian_mean_cost_value = 0.35 #0.05 #Cambiarlo per cambiare funzione di costo
    ...
    return clustered_image, Thermo_opt_flag
...
def  main_loop():
    ...
    # RGB_image = take_drone_camera_RGB_frame_simulation()
    # THERMO_image = take_drone_camera_frame_simulation()
    ...
    flag = False
    counter_optimization_completed = 0
    start = 0
    end = 0
    while(count < condition):
        ...
        if start_optimization_task == True and (RGB_opt_flag == False or Thermo_opt_flag == False):
            ...
            RGB_clustered_image, res, RGB_opt_flag, correlation_mask = RGB_optimization(RGB_args)
            end = time.time()

            print("OPT TIME: ", end - start) 
            ...
            RGB_opt_flag = True
            Thermo_opt_flag = True
            path_template = '/home/dji/DATA/dji_ws/user_image_selection_after_opt/correlation_mask/RGB_correlation_mask' + '.png' #'.png'  #ELiminare a regime
            correlation_mask = cv2.imread(path_template) 
           
            Op.corr_mask = correlation_mask   # Op_thermo.load_template(path_template)
            # Thermo_args = (THERMO, sd, Op_thermo, drone_obj, var_THERMO, Thermo_init_grid, Thermo_init_distance_grid,correlation_mask, n_better_solutions, THERMO_image, Thermo_opt_flag)
            # Thermo_clustered_image, Thermo_opt_flag = Thermal_optimization(Thermo_args)

            #Publish Optimized Parameters 
            publish_HSV_parameters(Op.h_low, Op.s_low, Op.v_low)
            print("PUBLISH HSV PARAMETERS --> H: {}, S:{}, V:{}".format(Op.h_low, Op.s_low,  Op.v_low))
             
            #PUBLISH Thermo Params
            ...
            publish_optimization_flag(flag = True)  #!!!!!! rimettere true 
            counter_optimization_completed = 0
            ...
            # publish_Thermo_parameters(args)
            print("PUBLISH HSV PARAMETERS --> H: {}, S:{}, V:{}".format(Op.h_low, Op.s_low,  Op.v_low))
            #Wait som,e seconds then come back inthe wait state to start a new optimization task
            # if counter_optimization_completed == 0:
            #     start = time.time()
            # else:
            #     end = time.time()
            #     #Se time > 10 secondi allora esce e riattende l'inizio di una nuova ottimizazzione 
            #     if end - start > 100:
            #         start_optimization_task = False
            #         RGB_opt_flag = False
            #         Thermo_opt_flag = False
            # counter_optimization_completed = counter_optimization_completed + 1
        else:
