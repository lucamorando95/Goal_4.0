...
//Publish Estimated line to meke it visible on visual frame 
 void publish_estimated_line(float a_estimated,  float c_estimated, float x_pos_I, float y_pos_I, Drone drone)
 {
  ...
  rotation_from_GF_to_BF(P1, x_pos_I, y_pos_I, drone.yaw_in_Rad);
  rotation_from_GF_to_BF(P2, x_pos_I, y_pos_I, drone.yaw_in_Rad);
   cout<<"drone.yaw_in_Rad: " << drone.yaw_in_Rad << endl;
   //Publish Point 1 
   drone.P1_B = P1;
   drone.P2_B = P2;
...
{
  ROS_INFO("##### target.latitude %f ....", target.latitude);
...
*/
 ...
 // #########################à INITIALIZATION OF KALMAN FILTER VARIABLES ###################
  ...
  float x_target_P2 =0.0;//Suppongo che il
  float y_target_P2 = 0.0;
  float x_target_P1_rotated = 0.0; //Solamente in esperimenti
  float y_target_P1_rotated = 0.0;
  float x_target_P2_rotated = 0.0;
  float y_target_P2_rotated = 0.0;
  float frame_rotation_rad = 0.0 * M_PI/180; //
 ...
 //Initialize position and coordinates 
  bool drone_position_init = true;
  ...
  // NOTA BENE: Gli assi di sta merda sono ruotati sono ruotati di 90 ghradi
  // Per riportare gli assi alla convenzione:
  // ^y
  // |
  // ||
  // |-------> x

  // per riportare alla convenzione 
  // x = y 
  //y = -x 
  while(nh.ok()) 
  {
  
  if (drone_position_init == true) 
  {
     ...
     cout<< "start_gps_location " <<start_gps_location.latitude << "start_gps_location.longitude << " << start_gps_location.longitude << endl;
     cout<< "drone.current_local_pos.x " << drone.current_local_pos.x << "drone.current_local_pos.y " << drone.current_local_pos.y << endl;
     
     ROS_INFO("##### start_gps_location.latitude %f ....", start_gps_location.latitude );
     ROS_INFO("##### start_gps_location.longitude %f ....", start_gps_location.longitude );
     ...
     x_target_P1 = drone.current_local_pos.y - start_local_position.y;//0.35;  //Le coordinate sono del punto P1 = (0.3, 0.2) rispetto al world frame fittizio
     y_target_P1 = -(drone.current_local_pos.x - start_local_position.x);
     x_target_P2 = x_target_P1 + 10;  //Suppongo che il punto P2 si trovi a 10 metri lungo la x del frame mondo, che è allineato al frame del drone quando si trova in P1. 
     y_target_P2 = -1*(drone.current_local_pos.y - start_local_position.y);
     x_target_P1_rotated = 0.0; //Solamente in esperimenti in lab pr far finat che i pannelli siano ruotati rispetto al frame
     y_target_P1_rotated = 0.0;
     x_target_P2_rotated = 0.0;
     y_target_P2_rotated = 0.0;
     frame_rotation_rad = 0.0 * M_PI/180; //angolo va definito in radianti: + rotazione in basso, - rotazione in alto--_> guardare note ipad

     cout<< "x target P1: " << x_target_P1 << "y target P1: " << y_target_P1 << endl;
     cout<< "x target P2: " << x_target_P2 << "y target P2: " << y_target_P2 << endl;
     
     target_point = 1; //Permette di definire la direzione del pannello
...
//   Initialize drone position in world frame 
     ...
     cout<< "a_init: " << a_init << "c_init: " << c_init;
     ...
     KF.yaw = drone.yaw_in_Rad;

  //Local offset ritorna distanza percorsa da punto di start
  localOffsetFromGpsOffset(localOffset, drone.current_gps, start_gps_location);
  ...
  if (drone.control_RGB_point1_x != control_RGB_point1_x_old || drone.control_RGB_point1_y !=  control_RGB_point1_y_old)
  {
  ...
  if (drone.control_RGB_point2_x != control_RGB_point2_x_old || drone.control_RGB_point2_y !=  control_RGB_point2_y_old)
  {
  ...
  //  cout<<"x_target_P2: "<<x_target_P2_rotated <<"  y_target_P2: " << y_target_P2_rotated<< endl;
   ...
   KF.translation_x = drone.local_pos_I.x;// x_respect_frame; //Translation camera respect Inertial frame
   KF.translation_y = drone.local_pos_I.y; //y_respect_frame;
 ...
 //Update KF with RGB observations 
  ...
  if (drone.control_thermo_point1_x != control_thermo_point1_x_old || drone.control_thermo_point1_y !=  control_thermo_point1_y_old)
  {
      ROS_INFO("##### P1 x %f ....", drone.control_thermo_point1_x);
      ROS_INFO("##### P1 y %f ....", drone.control_thermo_point1_y);
  ...
  //Take observation from Thermal camera 
  if (drone.control_thermo_point2_x != control_thermo_point2_x_old || drone.control_thermo_point2_y !=  control_thermo_point2_y_old)
  {
...
//Update KF with Thermal  observations if available
 ...
 //Print Kalman Filter estimated state value
  ...
  publish_estimated_line(KF.xh[0],  KF.xh[1], drone.local_pos_I.x, drone.local_pos_I.y, drone);
