  ...
  //cout << "[START MISSION]--> Reaching P1 panel "<< mission.count +1 << " --> distance --> " <<  mission.cartesian_distance_err<< endl;
   ...
   if (abs(drone.yaw_err) < 0.5 )
    ...
    if (mission.cartesian_distance_err < 0.2)
    {
        ...
        drone.THERMO_image_control_count = 100;
        drone.RGB_image_control_count = 100;
...
*/
   ...
   */
   ...
   if(drone.flagDroneThermoControlPoint1 == true && drone.flagDroneThermoControlPoint2 ==true && mission.panel_array_initialization == false ||  drone.THERMO_image_control_count < 50)
   {
      ...
      // Kalman_Filter.Kalman_Filter_calculate(  mission.x_target_P1_BF , mission.y_target_P1_BF ,  mission.x_target_P2_BF ,  mission.y_target_P2_BF , drone.control_Thermo_point1_x , drone.control_Thermo_point1_y , drone.control_Thermo_point2_x , drone.control_Thermo_point2_y  );
       ...
       //Display Observation IN GF
       float m = (drone.thermo_control_obs_P2_y_world -drone.thermo_control_obs_P1_y_world)/(drone.thermo_control_obs_P2_x_world -drone.thermo_control_obs_P1_x_world);
       float c = (-1*m *drone.thermo_control_obs_P1_x_world) + drone.thermo_control_obs_P1_y_world;

      cout << "[EKF THERMO ------->] Observation in GF:  a  " << m << " c : " << c<< endl; 
   ...
   if(drone.flagDroneRGBControlPoint1 == true && drone.flagDroneRGBControlPoint2 == true && mission.panel_array_initialization == false ||  drone.RGB_image_control_count< 500 )
   {
       ...
       //Display Observation IN GF
       float m = (drone.RGB_control_obs_P2_y_world -drone.RGB_control_obs_P1_y_world)/(drone.RGB_control_obs_P2_x_world -drone.RGB_control_obs_P1_x_world);
       float c = (-1*m *drone.RGB_control_obs_P1_x_world) + drone.RGB_control_obs_P1_y_world;
       cout << "[EKF RGB ------->] Observation in GF:  a  " << m << " c : " << c<< endl; 
...
//   //#####################GESTISCO IL RITORNO ALLA GPS LINE 
//    if (waypoints.error_from_GPS_line < 0.3 && coming_back_to_GPS_LINE ==true )
//    {
//        // Se distanza da GPS LINE < 0.3
//        // Se flag  coming_back_to_GPS_LINE ==true 
//        // se mission.navigation_iteration_count > 0 ---> generalmente è maggiore di zero perche vien eincremetato in fondo
//        // Entro solo per mettere L'iteratore a zero, e il flag di KF initialization a true.
//        //---> A questo punto il kf vine  reinizializzato e vengoino prese le osservazioni se disponibili. 
//        // Tuttavia il drone naviga ancora seguengìdo la GPS line
//        //NEcessario re- inizializzare KF

//        cout<< "[NAVIGATION] REQUIRED KF INITIALIZATION " << endl;
//        mission.navigation_iteration_count = 0;
//        mission.KF_Initialization = true;
//        coming_back_to_GPS_LINE = false;
...
//STOP NAVIGATION TO ACQUIRE SOME OBSERVATION OF THE PANELS AND COMPUTE A MORE PRECISE ESTIMATED STATE
if (stop_navigation_counter > 200 && stop_navigation_counter < 300)
{
    cout<< "STOP NAVIGATION TO ACQUIRE OBSERVATION: " << stop_navigation_counter << endl;
     drone.drone_vel_msg.linear.x = 0.0;
     drone.drone_vel_msg.linear.y =0.0;
}
...
if (mission.cartesian_distance_err < 1.5 || waypoints.error_from_GPS_line > 1.5)
{
    from_image = false;
    from_image_RGB = false;
    from_image_Thermo = false;
...
int main(int argc, char** argv)
{
   ...
   //PARAMETRS 
    ...
    Eigen::Vector2f panel_center_W(6.37, 2); //(12.74,2)
