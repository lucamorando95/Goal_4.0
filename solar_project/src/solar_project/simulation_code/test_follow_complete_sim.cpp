...
//Place panel centers:
void  place_panel_centers(Panel *panel, int n_panels, float distance_between_panels)
{
...
void place_panel_start_end_point(Panel *panel, int n_panels, float distance_between_panels, int configuration)
{
     ...
     if (KF.K_count < 10)
     {
      ...
      if (KF.K_count == 0 and KF.initialization == true)
      {
        ...
        else
        {
           ...
           {
             if (drone.drone_y  >  KF.quad_y_old) //Questa parte in implemnetazione con drone reale va nel main non nella classe
             {
      ...
      KF.thermal_update = true; //nel main
      //*************** MULTI DIM KF ************//
//-------------------  A Priori estimate of the current state x(t|t-1) = A*x(t-1|t-1))
    
      if  (KF_RGB.RGB_update == true) //Anche qusta parte va nel main condrone reale non nella classe
      {
