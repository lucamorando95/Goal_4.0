...
class Image_THERMO_elaboration:
    def __init__(self):
        ...
        self.desired_mask_width = 0.0
        self.mask_width = 0.0
        
        self.XT2_FOV = np.zeros(2)
        #Relative to Parameters Optimization
        self.parameters_optimization_enabled = False
    ...
    def first_cluster_search(self,contours, thresh, image):
        ...
        pixels_iniside_area = []
        region_pixels_size_coo = []
        self.count = 0

        for cnt in contours:
           
            if cv2.contourArea(cnt) < 500000 and cv2.contourArea(cnt) > self.th_3/3:
           ...
           #eva luate line angular coefficient:
               ...
               # Create a mask image that contains the contour filled in
                ...
                #cv2.line(img,(thresh.shape[1]-1,righty),(0,lefty),255,2)  
    ...
    def clustering_lines(self, row, col):
        ...
        for ii in range(0, len(self.start_point)):
            ...
            for jj in range(0,len(self.start_point)):
                # print("self.start_point[jj][1][0] {}, ii: {}".format(self.start_point[jj][1][0], ii))
                if ii == jj:
                    continue
                if self.start_point[jj][1][0] == ii:  #considero il valore di count
                    ...
                    #per verificare l'intersezione dov"rei fare un check sui valori di start ed end delle line sull'asse x, quindi sulle colonne
    ...
    def create_cluster_mask(self, cluster, image, thresh, drone_obj):
        ...
        for ii in range(0, len(cluster)):
            ...
            if (mask[mask.shape[0]/2][mask.shape[1]/2] == 250):
                white_array = []
                for ii in range(0, mask.shape[1]):
                    if (mask[0][ii] == 250):
                        white_array.append(ii)
                self.mask_width = len(white_array)
                white_array = []
    ...
    def voting_technique(self, line_versors, line_versors_old, thresh):
        ...
        points = []
        mean_lines_distance_from_center_line = []
        ...
        for ii in range(0, len(final_line_versors_frame)):
            ...
            for jj in range(0, thresh.shape[0]): #itero sulle righe ---> identifico la lista di punti che rappresenta la retta più vicina a quella centrale   
                if ((y - jj)*(1/(vy/vx))+x >= 0 and ((y - jj)*(1/(vy/vx))+x  <= thresh.shape[1])): #Cerco un valore di x quindi scorro su bordo orizzontale
                    ...
                    #print("point_line_distance_actual: ", point_line_distance_actual)
            ...
            if (math.isnan(np.mean(point_line_distance_actual))):
                ...
                for jj in range(0, thresh.shape[1]):#itero sulle colonne 
                    if (((jj-x)*vy/vx)+y >= 0 and ((jj-x)*vy/vx)+y <= thresh.shape[0]):
                        ...
                        dis = abs(xp-thresh.shape[1]/2)
                        point_line_distance_actual.append(dis) 
            if not point_line_distance_actual: 
                  point_line_distance_actual.append(1)
          
            mean_dis_curr_line = np.mean(point_line_distance_actual)
            
            mean_lines_distance_from_center_line.append([mean_dis_curr_line, ii])
            # with warnings.catch_warnings():
            #     warnings.simplefilter("ignore", category=RuntimeWarning)
                
            
            #print("points: ",    points)
            #print("mean_lines_distance_from_center_line: ", mean_lines_distance_from_center_line)
        return  point_line_distance_actual, mean_lines_distance_from_center_line, line_points
    ...
    def line_point_exportation_in_drone_body_frame(self, u1,v1,u2,v2, v0, u0, drone_obj, first):
        ...
        # gamma_1 = self.XT2_FOV[0]
        # gamma_2 = self.XT2_FOV[1]

        # gamma_1_rad = gamma_1 * M_PI/180
        # gamma_2_rad = gamma_2 * M_PI/180
        
        # #Triangle Base angle
        # alfa_1 = (180 - gamma_1)/2
        # alfa_1_rad = alfa_1 * M_PI/180
        # alfa_2 = (180 - gamma_2)/2
        # alfa_2_rad = alfa_2 * M_PI/180
      

        # #Obtain length of the big side of ground rectangle 
        # a1 = (np.sin((gamma_1_rad/2)/np.sin(alfa_1_rad))) * drone_obj.z
        # a1 = 2*a1
        # #Obtain length of the minor side of ground rectangle
        # a2 = (np.sin((gamma_2_rad/2)/np.sin(alfa_2_rad))) * drone_obj.z
        # a2 = 2*a2
        
        # #Obtain relation m/px given the altitude along the grgreate base of the ground image  ---> Asse Y drone 
        # r_y = a1/(u0*2)
        # r_x = a2/(v0*2)
        ...
        # yb1 = -1*gain_y*((u1  - u0 ) * r_y)
        # yb2 = -1*gain_y*((u2  - u0 ) * r_y)
        ...
        #Verificare se sulla y il gainy deve essere neativo
        
        #Point 1 
        #xb1 = -(const) * ((y1 * original_image_shape[1])/v0 -  original_image_shape[1])* drone_obj.z
        xb1 = -1*(const) * ((v1  -  v0 )* drone_obj.z)
        ...
        print('######### THERMO PANEL LINE RECOGNIZED, FOLLOWING')
