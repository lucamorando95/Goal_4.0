...
def image_pre_processing(RGB, image, line_versors_old, drone_obj):
    ...
    print("mean_distance_of_line_from_center_line: ", mean_distance_of_line_from_center_line)
...
def listen_keyboard_input(arg):
    ...
    print("arg.selection: ", arg.selection)
    plt.close('all')
    return

    
def show_images(RGB, dir_manual, dir_opt):

   #Create a thread to take input from keyboard
    try:
        thread = Thread(target = listen_keyboard_input, args = (RGB, ))
        thread.start()
        #thread.join()
    except:
...
def listener():
    ...
    while(count < condition):
        ...
        if RGB.counter_frame_analyzed > 10 and Matrice == True:
            ...
            if (data == True):
                ...
                start_point = (thresh.shape[1]/2, 0)
                end_point = (thresh.shape[1]/2, thresh.shape[0])
                cv2.line(image,start_point,end_point,255,2) 
                clustered_image = cv2.line(clustered_image, start_point, end_point, color, thickness)
