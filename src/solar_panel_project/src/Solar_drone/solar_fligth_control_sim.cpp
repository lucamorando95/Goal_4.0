...
struct Drone
{
  ...
  //Save Start Drone Position on the panel 
...
{
  int state = 0;
  int jump_panels_state_counter = 0;
  ...
  float x_target = 0.0;
  float y_target = 0.0;
  //Variable to use in jump_panels case
  ...
  float error_from_estimated_line_BF = 0.0;
  
  int count = 0;
  int navigation_iteration_count = 0;
  //Target for navigatiion
  float target_P2_x = 0.0;
  float target_P2_y = 0.0;
  ...
  bool evaluate_drone_yaw_des_for_jump_panels = false;
...
void  evaluate_control_point(bool from_image)
{
    ...
    //  c = drone.xh_[1];
     ...
     //Find distance point line: point is the body Origin and line is the line r
     double e = (abs(c)/(sqrt(pow(a,2) + pow(b,2))));
     mission.error_from_estimated_line_BF = e;
  ...
  //cout << "[START MISSION]--> Reaching P1 panel "<< mission.count +1 << " --> distance --> " <<  mission.cartesian_distance_err<< endl;
   ...
   if (abs(drone.yaw_err) < 0.5 )
    ...
    if (mission.cartesian_distance_err < 0.2)
    {
        ...
        drone.yaw_des = 0.0; //drone.drone_Yaw;
        ...
        mission.set_target(mission.x_target_P2, mission.y_target_P2);
...
//  if (stop_navigation_counter > 300)
//  {
//      drone.drone_vel_msg.linear.x = 0.0;
//      drone.drone_vel_msg.linear.y = 0.1* sin(stop_navigation_counter);
//  }
...
if (KF_init == true)
  {
    ...
    //drone.yaw_des = 0.0; atan2(drone.y_target, drone.x_target); //Align yaw of the drone with the direction of the line--- In matrice yaw des should be the yaw after the user selection
    ...
    //Da salvare anche su Matrice
    drone.start_position_x = drone.drone_x;
    drone.start_position_y = drone.drone_y;
    from_image = false;
    mission.panel_array_initialization = false;

  }

//Check drone yaw to obtain a precise control
if (drone.yaw_des > 0)
{
...
{
    drone.drone_vel_msg.angular.z  = pid_yaw->position_control_knowing_velocity(drone.yaw_des, -1*abs(drone.drone_Yaw) , 0, drone.drone_ang_vel_z); //ci andrebbe yaw
   if (drone.drone_Yaw > 0)
  {
    drone.drone_vel_msg.angular.z  = -1*  drone.drone_vel_msg.angular.z ;
  }
...
cout << "YAW DES: " << drone.yaw_des << " drone.drone_Yaw: "<< drone.drone_Yaw << endl; 
...
//Take te navigation target relative to the end of the solar array
cout<<"mission.target_P2_x: " << mission.target_P2_x << endl;
cout<<"mission.target_P2_y: " << mission.target_P2_y << endl;
mission.cartesian_distance_err = sqrt(pow(mission.target_P2_x- drone.drone_x,2) + pow(mission.target_P2_y- drone.drone_y,2)); //distanza dal raggiungere la fine del oannello
...
void evaluate_GPS_offset_for_new_array_starting_point()
{
    //Obtain GPS waypoints of previous panel end point
    mission.x_target_P1 = waypoints.waypoints_x_coo_world_frame[mission.count - 1];
    mission.y_target_P1 = waypoints.waypoints_y_coo_world_frame[mission.count - 1];
    
    //Obtain GPS waypoints of successive panel start point
    mission.x_target_P2 = waypoints.waypoints_x_coo_world_frame[mission.count];
    mission.y_target_P2 = waypoints.waypoints_y_coo_world_frame[mission.count];
    
    //Obtain GPS waypoints of previous panel start point
    float previous_x_start = waypoints.waypoints_x_coo_world_frame[mission.count - 2];
    float previous_y_start = waypoints.waypoints_y_coo_world_frame[mission.count - 2];

     
    
    //Evaluate angle between previous_start, P1, P2
    float a =  sqrt(pow(mission.x_target_P1 - previous_x_start, 2) + pow(mission.y_target_P1 - previous_y_start, 2)); //distance between previous start e P1
    ...
    float c = sqrt(pow(mission.x_target_P2 - previous_x_start, 2) + pow(mission.y_target_P2 -  previous_y_start, 2)); //distance between opreviosu start e P3
    
    cout<<"a: " << a << "b: " << b << "c: " << c<< endl;
    
    /* Find angle gamma which belive between previous_x, P1, P2 respectively (start actual array, end actual array, start next array) */
    //Find angles alfa, beta,gamma using the cosine rule 
    //alfa = angle between a and c
    // beta = angle between c and b
    ...
    if (mission.jump_panels_state_counter < 5)
    {
        Rotation_GF_to_BF_des_pos(mission.x_target_P2, mission.y_target_P2,  drone.drone_Yaw);
        mission.x_target_P2_B = check_x_b;
        mission.y_target_P2_B = check_y_b;
        
        //Obtain B coo of the previous start wayooint
        Rotation_GF_to_BF_des_pos(previous_x_start, previous_y_start,  drone.drone_Yaw);
        float x_target_P1_B = check_x_b;
        float y_target_P1_B = check_y_b;
    }
    
   
    
    //Distance done from drone last position 
    float distance_done =  sqrt(pow(drone.drone_x - drone.last_position_x, 2) + pow(drone.drone_y- drone.last_position_y, 2));
    ...
    cout << "y_target_P2_B: " << mission.y_target_P2_B << endl;
    ...
    cout<<" mission.x_target_P2_BF: " <<  mission.x_target_P2_BF << " mission.y_target_P2_BF: " <<  mission.y_target_P2_BF << endl;
    cout << "drone x: " << drone.drone_x << "drone y: " << drone.drone_y << endl;
...
void jump_panels_array(Panel *panel,  PID *pid_x, PID *pid_y, PID *pid_z, PID *pid_yaw)
{
    ...
    //Evaluate Starting of new panel point via GPS waypoints offset 
    evaluate_GPS_offset_for_new_array_starting_point();
    ...
    // mission.x_target_P2 =  panel->obtain_xcoo_panel_world_frame()[mission.count]; //P2 diventa il sucessivi punto di inizio pannello
    // mission.y_target_P2 =  panel->obtain_ycoo_panel_world_frame()[mission.count];
    
    // cout<<"REAL START PANEL POSITION X: " << panel->obtain_xcoo_panel_world_frame()[mission.count]  << "Y: " << panel->obtain_ycoo_panel_world_frame()[mission.count] << endl;
    // cout<<"Evaluated Start X: " << mission.x_target_P2  << "Y: " <<  mission.y_target_P2 << endl;



    //cout << "mission.x_target_P2:  " << mission.x_target_P2 << "mission.y_target_P2:  " << mission.y_target_P2 << endl;
    if (drone.yaw_des < 0 &&  drone.drone_Yaw > 0 || drone.yaw_des > 0 &&  drone.drone_Yaw < 0)
    {
        drone.drone_Yaw = -1* drone.drone_Yaw;
    }
    float yaw_err = drone.yaw_des - drone.drone_Yaw;
    if (abs(yaw_err) < 0.2)
    {
         drone.drone_vel_msg.angular.z = 0.0;
    } 
    else
    {
          drone.drone_vel_msg.angular.z  = pid_yaw -> position_control_knowing_velocity(drone.yaw_des , drone.drone_Yaw, 0, drone.drone_ang_vel_z);
    }   
    ...
    drone.drone_vel_msg.linear.x = pid_x->calculate(mission.x_target_P2_BF, drone.drone_x_b);
    drone.drone_vel_msg.linear.y = pid_y->calculate(mission.y_target_P2_BF, drone.drone_y_b);
    drone.drone_vel_msg.linear.z = pid_z->position_control_knowing_velocity(drone.last_position_z, drone.drone_z, 0, drone.drone_lin_vel_z);

    //Error distance expressed in Body frame in this case
    
    mission.cartesian_distance_err = sqrt(pow(mission.x_target_P2_BF,2) + pow(mission.y_target_P2_BF,2)); 
    cout << "mission.cartesian_distance_err: " << mission.cartesian_distance_err << endl;
  
    if (mission.cartesian_distance_err < 0.2 && abs(yaw_err) < 0.2)
    {
      mission.set_target(waypoints.waypoints_x_coo_world_frame[mission.count + 1], waypoints.waypoints_y_coo_world_frame[mission.count + 1]);
      mission.count = mission.count + 1;
      drone.image_control_count = 100; // inizializazzione valore grosso. --> Serve per aggiornare il filtro per un certo numero di iterazioni anche se nin sono state ricevute nuove osservazioni-- > utilizzal'ultima osservazione ricevuta 
      drone.THERMO_image_control_count = 200;
      drone.RGB_image_control_count = 200;
      ...
      mission.jump_panels_state_counter = 0;
   ...
   mission.jump_panels_state_counter = mission.jump_panels_state_counter + 1;
}
...
void yaw_alignment_for_array_jump(PID *pid_x, PID *pid_y, PID *pid_z, PID *pid_yaw)
{
    if (mission.evaluate_drone_yaw_des_for_jump_panels == true)
    {
        //ROtate in BF psoition start position of the drone in the previous array
        cout<<"drone.start_position_x: " << drone.start_position_x << "   drone.start_position_y: " <<  drone.start_position_y << endl;
        Rotation_GF_to_BF_des_pos(drone.start_position_x, drone.start_position_y,  drone.drone_Yaw);
        drone.yaw_des = atan2(check_y_b, check_x_b);
        drone.yaw_des = abs(drone.drone_Yaw) - abs(drone.yaw_des);
        if (drone.yaw_des > M_PI || drone.yaw_des < -M_PI)
        {
            drone.yaw_des = M_PI;
        }
        mission.evaluate_drone_yaw_des_for_jump_panels = false;
    }
    cout<< "[YAW ALIGNMENT]" << endl;
      // Yaw Alignment with new array when the drone approaches point P2
    cout<<"drone.yaw_des" << drone.yaw_des << "  drone Yaw: " << drone.drone_Yaw << endl;
    drone.drone_vel_msg.angular.z  = pid_yaw -> position_control_knowing_velocity(drone.yaw_des , drone.drone_Yaw, 0, drone.drone_ang_vel_z);
...
int main(int argc, char** argv)
{
    ...
    Eigen::Vector2f panel_center_W(6.37, 2); //(12.74,2) //6.37, 2)
    float theta = 0; //45.0 * M_PI/180;
    float size = 1.0;
    float length = 11; //22 11
...
Kalman_Filter.pass_to_class_initialization_matrices(Px, R, pass_to_class_initialization_matrices);
    ...
    //WRITE ON TXT simulation_data/sim_data_complete
        ...
        std::ofstream outFile35("simulation_data/sim_data_complete/error_from_estimated_line_BF.txt");
     ...
     /*##################################################################################################################################*/
       ...
       // Maintain desired altitude and attitude
        ...
        switch(mission.state)
        {
            ...
            case 4:
               yaw_alignment_for_array_jump(&pid_x, &pid_y, &pid_z, &pid_yaw);
        }
...
outFile35 << mission.error_from_estimated_line_BF <<"\n";
