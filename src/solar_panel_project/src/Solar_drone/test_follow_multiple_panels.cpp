...
struct Panel {
    ...
    //Setpoint vector along the line of the panel
    vector <float> setpoint_x;
    vector <float> setpoint_y;
};
...
void evaluate_setpoint_vector(Panel* panel, int count)
{
    ...
    x = panel->xcoo_panel_world_frame[count] - drone.drone_x; 
    y = panel->ycoo_panel_world_frame[count] - drone.drone_y; 
    ...
    for (int i = 0; i < panel->length; i ++)
    {
       ...
       panel->setpoint_x.push_back(a);
       panel->setpoint_y.push_back(b);
...
int main(int argc, char** argv)
{
   ...
   */
    ...
    while(nh.ok()) {
        ...
        {
            //Reaching Point End P2
            if (flag_create_setpoint == true)
            {
                //Create set point along the panel to follow the traectory alongo the line
                evaluate_setpoint_vector(&panel, count);
                count_setpoint = 0;
                
                //Select first setpoint as target
                x_target = panel.setpoint_x[count_setpoint];
                y_target = panel.setpoint_y[count_setpoint];
                cout<<"x_target: "<<x_target<<endl;
                cout<<"y_target: "<<y_target<<endl;
                flag_create_setpoint = false;
            }
            ...
            cartesian_distance_err1 = sqrt(pow(x_target - drone.drone_x,2) + pow(y_target- drone.drone_y,2));
            ...
            Rotation_GF_to_BF_drone_pos(drone.drone_Yaw);
             ...
             drone_vel_msg.linear.y = pid_y.calculate(check_y_b, drone.drone_y_b);
              ...
              if (cartesian_distance_err1 < 0.40)
              {
                count_setpoint  = count_setpoint + 1;  
                if (count_setpoint >= (panel.setpoint_x.size() - 1))
                {
                    x_target = panel.setpoint_x[count_setpoint];
                    y_target = panel.setpoint_y[count_setpoint];
                }
                else 
                {
                    x_target = panel.xcoo_panel_world_frame[count];
                    y_target = panel.ycoo_panel_world_frame[count];
                    panel.setpoint_x.clear();
                    panel.setpoint_y.clear();
