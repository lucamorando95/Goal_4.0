...
class KF_Impl
{
    ...
    void pass_to_class_initialization_matrices(Eigen::Matrix2f Px, Eigen::Matrix2f R, float des_waiting_value);
...
void KF::pass_to_class_initialization_matrices(Eigen::Matrix2f Px, Eigen::Matrix2f R, float des_waiting_value)
{
    return KF_implementation -> pass_to_class_initialization_matrices(Px, R, des_waiting_value);
}
...
{
  /*
  FInd the angle Gamma relative to the difference given by the orientation of the line 
  in GF and the orientation of the drone (required to find the orientation of the line a respectin drone BF)
  */
...
/*
Implementation of the Equation of the EKF
*/
                                ...
                                float X_drone_GF, float Y_drone_GF, float Yaw_drone_GF)
{
   ...
   //Define sitution when the KF is not updated
   if (K_count < des_numb_of_waiting_it && initialization == true)
    {
    ...
    else
    {
     ...
     if (a_obs_GF < xh_[0] - 0.9|| a_obs_GF >  xh_[0] + 0.9 )
    {
       //Eliminate very wrong predictions
        cout<< "[KF INFO] VERY WRONG a^L observation---> EXITING FROM KF: " << xh_[0] << ", " << a_obs_GF <<endl;
    ...
    if (c_obs_GF < xh_[1] - 5 || c_obs_GF > xh_[1] + 5)
    {
        ...
        cout<< "[KF INFO] VERY WRONG c^L observation---> EXITING FROM KF: " << xh_[1] << ", " << c_obs_GF <<  endl;
    ...
    //FInd the associated angle relative to the orientation Yaw of the drone respect the orientation of the line (check on IPAD)
...
void KF_Impl::pass_to_class_initialization_matrices(Eigen::Matrix2f Px, Eigen::Matrix2f R, float des_waiting_value)
{
     
     _Px = Px;
     _R = R;
     des_numb_of_waiting_it = des_waiting_value;
