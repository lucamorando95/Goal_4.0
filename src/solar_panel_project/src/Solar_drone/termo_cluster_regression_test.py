...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 22 09:50:09 2020
...
sys.path.remove('/home/lucamora/catkin_ws/devel/lib/python2.7/dist-packages')
import cv2
import time
import os
import math
...
def line_point_exportation_in_drone_body_frame(u1,v1,u2,v2, v0, u0, drone_obj, first):
   ...
   print('u1: {0} v1: {1} u2: {2} v2: {3}'.format(u1,v1,u2,v2))
   ...
   yb1 = gain* (const) * ((u1  - u0 ) * drone_obj.z)
   #Point2 
   xb2 = -(const) * ((v2 - v0 )* drone_obj.z)
   yb2 = gain* (const) * ((u2 - u0  )* drone_obj.z)
...
def image_pre_processing(image, line_versors_old,drone_obj):
    ...
    final_line_versors_frame = line_versors
    ...
    for ii in range(0, len(final_line_versors_frame)):
        ...
        # print('thresh.shape[1]/2: {0}'.format(thresh.shape[1]/2)) 
        for jj in range(0, thresh.shape[1]): #itero sulle colonne  
               if (((jj-x)*vy/vx)+y >= 0 and ((jj-x)*vy/vx)+y <= thresh.shape[0]):
                       ...
                       dis = abs(xp-thresh.shape[1]/2) #distanza del pixel da quelli che compongono la retta centrale
...
def load_image(photo_number):
     ...
     clustered_image, line_versors = image_pre_processing(image, line_versors_old, drone_obj)
...
def listener():
    ...
    while(count < condition):
        ...
        rospy.loginfo('MAIN LOOP: Loop time: %f ', end2)
