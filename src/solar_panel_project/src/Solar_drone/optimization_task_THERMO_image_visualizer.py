...
class RGB_image_feature:
    ...
    def callback_THERMO(self, ros_data):
        ...
        self.clustered_image_manual = take_drone_THERMO_clustered_image()
        
        cv2.imshow('1. Manual Image: ', self.clustered_image_manual)
        cv2.waitKey(1)

        cv2.imshow('2. Optimized Image : ', self.image_opt)
        cv2.waitKey(1)
