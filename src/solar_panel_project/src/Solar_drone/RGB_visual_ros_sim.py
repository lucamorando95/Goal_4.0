...
th5 = 70000#51200#70000  #Area oltre il quale il cluster è considerato unico
th6 = 0.5 #Magnitude value for clustering
index_th1 = 0
index_th2 = 0
index_th3 = 0
index_th4 = 0
    ...
    if len(same_cluster) > 0:
            ...
            if len(same_cluster_similar_direction) > 0:
                ...
                for jj in range(0, len(same_cluster)):
                   # print("same_cluster[jj]: ", same_cluster[jj])
                    for kk in range(0, len(same_cluster[jj])):
                        new_list.append(same_cluster[ii])
    ####################                
...
def image_pre_processing(image,line_versors_old,  drone_obj, counter_frame_analyzed, counter_frame_received):
    ...
    debug_image_publication = True  ##########################   DEBUG
    ...
    th3_value = th3
    th4_value = th4
    #imghsv1 = cv2.cvtColor(image, cv2.COLOR_HSV2BGR) 
    # cv2.imshow('RGB', imghsv1)
    # cv2.waitKey(1)
...
#---> Load parameters
    ...
    lower_blue = np.array([H_MIN,S_MIN,V_MIN], np.uint8)
    upper_blue = np.array([H_MAX,S_MAX,V_MAX], np.uint8)

    #IN SIMULAZIONE VALORI
    # lower_blue = np.array([80,10,10])
    # upper_blue = np.array([180,180,130])

  

    #---> Valori Pannello reale 
    # lower_blue = np.array([90,70,20], np.uint8)  #Valori pannello in lab [80,0,40]
    # upper_blue = np.array([170,140,220], np.uint8) #[160,100,220]
...
def load_RGB_image(photo_number):
     name = '/home/lucamora/image_analysis/Volo_01_RGB/DJI_00' + str(photo_number) + '.png'
...
def listener():
    ...
    # th3 = rospy.get_param("/RGB_params/th3")
    # th4 = rospy.get_param("/RGB_params/th4")
    # th5 = rospy.get_param("/RGB_params/th5")
    # th6 = rospy.get_param("/RGB_params/th6")
