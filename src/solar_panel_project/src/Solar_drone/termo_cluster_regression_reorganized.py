...
class drone(object):
    ...
    def __init__(self):
        #Store position data
        self.x = 0 #pose.pose.pose.position.x
        self.y =  0 # pose.pose.pose.position.y
...
def image_pre_processing(THERMO,image, line_versors_old,drone_obj):
    ...
    cv2.waitKey(1)
    # cv2.imshow('thresh_blur2', thresh_blur2)
    # cv2.waitKey(1)
    
    
    # for ii in range(0,thresh_blur2.shape[0]):
    #     for jj in range(0,thresh_blur2.shape[1]):
    #         if (thresh_blur2[ii][jj] == 255):
    #             thresh_blur2[ii][jj] = 10
    
            
    # for ii in range(0,thresh_blur2.shape[0]):
    #     for jj in range(0,thresh_blur2.shape[1]):
    #         if (thresh_blur2[ii][jj] == 0):
    #             thresh_blur2[ii][jj] = 255
     
    # for ii in range(0,thresh_blur2.shape[0]):
    #     for jj in range(0,thresh_blur2.shape[1]):
    #         if (thresh_blur2[ii][jj] == 10):
    #             thresh_blur2[ii][jj] = 0
...
def load_image(photo_number):
     ...
     image = image[0:image.shape[0], image.shape[1]/2:image.shape[1]]
...
def listen_keyboard_input(THERMO, dir_manual, dir_opt):
...
def show_images(THERMO, dir_manual, dir_opt, drone_obj, clustered_image):
   ...
   #Create a thread to take input from keyboard
    try:
        thread = Thread(target = listen_keyboard_input, args = (THERMO, dir_manual, dir_opt,))
        thread.start()
        #thread.join()
    except:
